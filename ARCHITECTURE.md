...
* **`utils/`**
  * File-file pembantu (*helper*). Contoh: `currency_formatter.dart` (mengubah angka jadi format "Rp"), `date_utils.dart` (format tanggal), dan `tax_calculator.dart` (rumus hitung pajak/servis).
* **`widgets/`**
  * Komponen UI yang sering dipakai berulang-ulang di berbagai layar. Contoh: `hover_scale.dart` (efek tombol mengecil saat ditekan) dan `table_panel.dart`.
...
* **`services/`**: `payment_api_service.dart` (API untuk memproses mutasi uang dan integrasi Midtrans).
