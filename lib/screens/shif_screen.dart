...
class _ShiftScreenState extends State<ShiftScreen> {
    ...
    if (activeShift != null && activeShift.masterInfo != null) {
      try {
        ...
        final DateTime fullShiftEndTime = DateTime(
          ...
          shiftEndToday.hour, shiftEndToday.minute, shiftEndToday.second
        );
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: const Text("Lanjutkan Tutup Shift", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              ),
            ),
          ],
        ),
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<dynamic>>(
        ...
        builder: (context, snapshot) {
              ...
              try {
                ...
                if (orderTime.isAfter(startShiftWIB)) {
                  ...
                  if (paymentAmount.containsKey(method)) {
                    paymentAmount[method] = (paymentAmount[method] ?? 0) + orderTotal;
                  }
                  for (var item in order.items) {
                    productSalesQty[item.itemName] = (productSalesQty[item.itemName] ?? 0) + item.quantity;
                  }
                }
              } catch (e) {
          ...
          var filteredProducts = productSalesQty.entries.where((e) => e.value >= 10).toList()..sort((a, b) => b.value.compareTo(a.value));
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                _buildProfileHeader(context, cashierName, activeShift, kasAwal, count, bersih),
                const SizedBox(height: 24),
                LayoutBuilder(
                  builder: (context, constraints) {
                    if (constraints.maxWidth > 800) {
                      return Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Expanded(flex: 6, child: _buildProdukTerlaris(topProducts)),
                          const SizedBox(width: 24),
                          Expanded(flex: 4, child: _buildMetodeBayar(paymentAmount, bersih)),
                        ],
                      );
                    } else {
                      return Column(
  ...
  Widget _buildProfileHeader(BuildContext context, String name, RekapShift? shift, int awal, int count, int bersih) {
    String loginTime = shift?.startedAt != null ? DateFormat('HH:mm').format(shift!.startedAt!.toLocal()) : "--:--";
                        ...
                        if (shift != null && shift.status != 'none')
                          Container(
                            ...
                            decoration: BoxDecoration(
                              color: isActuallyLate ? Colors.red.shade50 : Colors.green.shade50,
                              borderRadius: BorderRadius.circular(6),
                              border: Border.all(color: isActuallyLate ? Colors.red : Colors.green),
                            ),
                            child: Text(isActuallyLate ? "Telat" : "Tepat Waktu", style: TextStyle(color: isActuallyLate ? Colors.red : Colors.green, fontSize: 10, fontWeight: FontWeight.bold)),
                          ),
                      ],
                    ),
                    Text("${shift?.masterInfo?.name ?? '-'} • Masuk: $loginTime WIB", style: AppStyle.subTitleText),
                  ],
                ),
              ),
              IconButton(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          if (topProducts.isEmpty) const Expanded(child: Center(child: Text("Belum ada produk mencapai 10 porsi", style: TextStyle(color: Colors.grey))))
          else Expanded(
              ...
              child: Column(
                children: topProducts.asMap().entries.map((mapEntry) {
                  ...
                  return Padding(
                    ...
                    child: Row(
                      children: [
                        CircleAvatar(radius: 18, backgroundColor: AppStyle.primaryBlue, child: Text("${index + 1}", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
                        const SizedBox(width: 16),
  ...
  Widget _buildMetodeBayar(Map<String, int> paymentAmount, int totalMoney) {
    final Map<String, Color> methodColors = {"CASH": Colors.blue, "CARD": Colors.orange, "QRIS": Colors.red};
    return Container(
      height: 380,
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24)),
            ...
            child: SingleChildScrollView(
              child: Column(
                children: paymentAmount.entries.map((entry) {
                  final double percentage = totalMoney > 0 ? (entry.value / totalMoney) : 0;
                  final Color color = methodColors[entry.key.toUpperCase()] ?? Colors.grey;
                  return Padding(
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            ...
                            Text("Rp ${_formatNumber(entry.value)}", style: const TextStyle(fontWeight: FontWeight.bold)),
                          ],
                        ),
                        const SizedBox(height: 8),
                        LinearProgressIndicator(value: percentage, backgroundColor: Colors.grey.shade100, valueColor: AlwaysStoppedAnimation<Color>(color), minHeight: 8),
                      ],
                    ),
                  );
                }).toList(),
