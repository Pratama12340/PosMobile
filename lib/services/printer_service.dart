...
class TerminalPrinterService {
  ...
  void printToTerminal(TransactionModel transaction) {
    ...
    for (var tax in transaction.taxBreakdown) {
      ...
      double taxAmt = double.tryParse(tax['calculated_amount']?.toString() ?? '0') ?? 0;
      if (taxAmt > 0) receipt.writeln(_formatRow(taxName, "Rp ${_fMoney(taxAmt)}"));
    }
    receipt.writeln("-" * 32);
  ...
  void printKitchenToTerminal(TransactionModel transaction) {
    ...
    kitchen.writeln("Meja  : ${transaction.tableNumber.isEmpty ? "-" : transaction.tableNumber}");
    kitchen.writeln("Cust  : ${transaction.customerName.isEmpty ? "-" : transaction.customerName}");
    kitchen.writeln("-" * 30);

    for (var item in transaction.items) {
      kitchen.writeln(_formatRow(item.itemName.toUpperCase(), "x${item.quantity}", width: 30));
